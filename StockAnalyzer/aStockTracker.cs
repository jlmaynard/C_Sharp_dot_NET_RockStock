...
namespace StockAnalyzer
{
   ...
   public class aStockTracker
   {
      ...
      /// </summary>
      public aStockTracker(string Ticker, string StartingMonth, string StartingDay,
      ...
      public aStockTracker(string Ticker, DateTime StartingDate, DateTime EndingDate, string Frequency)
      {
         this.Ticker = Ticker;
      ...
      /// <summary>
      /// Calculates the Ten day avg for Kelli
      /// </summary>
      public double TenDayAvg
      {
         get
         {
      ...
      /// <summary>
      /// This method finds high and low values with LINQ 
      /// </summary>
      /// <param name="table">DataTable</param>
      public void calcHighLowWithLINQ(DataTable table)
      {
         try
         {
            List<double> highLevels = table.AsEnumerable().Select(high => (high.Field<double>("High"))).Distinct().ToList();
            // Of all the high values find the max of those for the HighValue
            HighValue = highLevels.Max();

            List<double> lowLevels = table.AsEnumerable().Select(low => (low.Field<double>("Low"))).Distinct().ToList();
            LowValue = lowLevels.Min();
         }
         catch (Exception ex)
         {
            Console.WriteLine("This is in calcHighLowWithLINQ\n" + ex.Message);
         }
      }

      public double calcTenDayAvg(DataTable table)
      {
         // Pull out the closing values column
         List<double> closingLevels = table.AsEnumerable().Select(low => (low.Field<double>("Close"))).Distinct().ToList();

         // Set up list to store last ten days from closing values to calc avg from
         List<double> lastTenDays = new List<double>();

         // Populate the last ten days list
         for (int row = 0; row < 10; row++)
         {
            lastTenDays.Add(closingLevels[row]);
         }

         // Get the avg from last ten days

         return TenDayAvg = lastTenDays.Average();
      }
